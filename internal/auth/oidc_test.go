...
func TestOIDCVerifierDiscoveryURL(t *testing.T) {
	...
	v, err := NewOIDCVerifier(ctx, issuer, metadataURL, "mcp-docstore", "email", "groups", "off", 15*time.Second)
	require.NoError(t, err)
...
func TestOIDCVerifierDiscoveryURLRejectsIssuerMismatch(t *testing.T) {
	...
	_, err := NewOIDCVerifier(ctx, "https://wrong.example.com", metadataURL, "mcp-docstore", "email", "groups", "off", 15*time.Second)
	require.Error(t, err)
}
...
func TestOIDCVerifierDiscoveryURLRejectsMissingDocument(t *testing.T) {
	...
	_, err := NewOIDCVerifier(ctx, issuer, issuer+"/.well-known/does-not-exist", "mcp-docstore", "email", "groups", "off", 15*time.Second)
	require.Error(t, err)
}
...
func TestOIDCVerifierVerifiesAndExtractsClaims(t *testing.T) {
	...
	v, err := NewOIDCVerifier(ctx, issuer, "", "mcp-docstore", "email", "groups", "off", 15*time.Second)
	require.NoError(t, err)
...
func TestOIDCVerifierRequiresAudience(t *testing.T) {
	issuer, _ := startOIDC(t)
	_, err := NewOIDCVerifier(context.Background(), issuer, "", "", "email", "groups", "off", 15*time.Second)
	require.Error(t, err)
}
...
func TestOIDCVerifierEmailVerifiedPolicy(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.policy+"/"+claimName(tc.claim), func(t *testing.T) {
			v, err := NewOIDCVerifier(ctx, issuer, "", "mcp-docstore", "email", "groups", tc.policy, 15*time.Second)
			require.NoError(t, err)
...
func TestOIDCVerifierDiscoveryTimesOut(t *testing.T) {
	// A server that blocks forever on the discovery request: with a bounded client the
	// verifier construction must fail (not hang) within ~the timeout.
	block := make(chan struct{})
	t.Cleanup(func() { close(block) })
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Block until the bounded client cancels the request (its deadline fires) or test
		// cleanup closes block. Selecting on the request context is essential: without it
		// the handler goroutine stays parked on <-block and httptest.Server.Close (a LIFO
		// cleanup that runs before close(block)) waits on it forever, hanging the test.
		select {
		case <-r.Context().Done():
		case <-block:
		}
	...
	start := time.Now()
	// Group A added the email_verified_policy param before discoveryTimeout; pass "off" here.
	_, err := NewOIDCVerifier(context.Background(), ts.URL, "", "mcp-docstore", "email", "groups", "off", 200*time.Millisecond)
	require.Error(t, err)
