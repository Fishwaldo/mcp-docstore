...
type ProjectQuery struct {
	...
	withGroupShares *ProjectGroupShareQuery
...
func (_q *ProjectQuery) QueryTenant() *TenantQuery {
...
// QueryGroupShares chains the current query on the "group_shares" edge.
func (_q *ProjectQuery) QueryGroupShares() *ProjectGroupShareQuery {
	query := (&ProjectGroupShareClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withGroupShares: _q.withGroupShares.Clone(),
...
func (_q *ProjectQuery) WithTenant(opts ...func(*TenantQuery)) *ProjectQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
// WithGroupShares tells the query-builder to eager-load the nodes that are connected to
// the "group_shares" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProjectQuery) WithGroupShares(opts ...func(*ProjectGroupShareQuery)) *ProjectQuery {
	query := (&ProjectGroupShareClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupShares = query
	return _q
}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			_q.withOwner != nil,
			_q.withShares != nil,
			_q.withGroupShares != nil,
			_q.withDocuments != nil,
		}
	)
	if _q.withTenant != nil || _q.withOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withGroupShares; query != nil {
		if err := _q.loadGroupShares(ctx, query, nodes,
			func(n *Project) { n.Edges.GroupShares = []*ProjectGroupShare{} },
			func(n *Project, e *ProjectGroupShare) { n.Edges.GroupShares = append(n.Edges.GroupShares, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Project, init func(*Project), assign func(*Project, *Tenant)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Project)
	for i := range nodes {
		if nodes[i].tenant_projects == nil {
			continue
		}
		fk := *nodes[i].tenant_projects
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_projects" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProjectQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Project, init func(*Project), assign func(*Project, *User)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_owner" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProjectQuery) loadGroupShares(ctx context.Context, query *ProjectGroupShareQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectGroupShare)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.ProjectGroupShare(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.GroupSharesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_group_shares
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_group_shares" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_group_shares" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
