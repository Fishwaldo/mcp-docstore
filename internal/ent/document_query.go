...
type DocumentQuery struct {
	...
	withSnapshots *DocumentSnapshotQuery
	withFKs       bool
...
func (_q *DocumentQuery) QueryCreatedBy() *UserQuery {
...
func (_q *DocumentQuery) QueryUpdatedBy() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *DocumentQuery) QuerySnapshots() *DocumentSnapshotQuery {
	query := (&DocumentSnapshotClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *DocumentQuery) WithCreatedBy(opts ...func(*UserQuery)) *DocumentQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DocumentQuery) WithUpdatedBy(opts ...func(*UserQuery)) *DocumentQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DocumentQuery) WithSnapshots(opts ...func(*DocumentSnapshotQuery)) *DocumentQuery {
	query := (&DocumentSnapshotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if _q.withProject != nil || _q.withCreatedBy != nil || _q.withUpdatedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withCreatedBy; query != nil {
		if err := _q.loadCreatedBy(ctx, query, nodes, nil,
			func(n *Document, e *User) { n.Edges.CreatedBy = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withUpdatedBy; query != nil {
		if err := _q.loadUpdatedBy(ctx, query, nodes, nil,
			func(n *Document, e *User) { n.Edges.UpdatedBy = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSnapshots; query != nil {
		if err := _q.loadSnapshots(ctx, query, nodes,
			func(n *Document) { n.Edges.Snapshots = []*DocumentSnapshot{} },
			func(n *Document, e *DocumentSnapshot) { n.Edges.Snapshots = append(n.Edges.Snapshots, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DocumentQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*Document, init func(*Document), assign func(*Document, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Document)
	for i := range nodes {
		if nodes[i].document_created_by == nil {
			continue
		}
		fk := *nodes[i].document_created_by
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "document_created_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *DocumentQuery) loadUpdatedBy(ctx context.Context, query *UserQuery, nodes []*Document, init func(*Document), assign func(*Document, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Document)
	for i := range nodes {
		if nodes[i].document_updated_by == nil {
			continue
		}
		fk := *nodes[i].document_updated_by
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "document_updated_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *DocumentQuery) loadSnapshots(ctx context.Context, query *DocumentSnapshotQuery, nodes []*Document, init func(*Document), assign func(*Document, *DocumentSnapshot)) error {
	...
	query.withFKs = true
	query.Where(predicate.DocumentSnapshot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(document.SnapshotsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.document_snapshots
		if fk == nil {
			return fmt.Errorf(`foreign-key "document_snapshots" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "document_snapshots" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
