...
- **Per-request identity rides `req.Extra.TokenInfo`, NOT `ctx`, on the `/mcp` transport.** With Streamable HTTP a session spans many requests and the tool-handler `ctx` is the *session-connect* ctx. The SDK `auth.RequireBearerToken` middleware attaches the verified `*auth.TokenInfo` (carrying our `store.Identity`) to each request's `RequestExtra`. Always read identity via `auth.IdentityFromRequest(req)` in production; never from `ctx`. `/api` has no such wrinkle — `internal/web.RequireBearer` stamps identity straight onto the request `ctx` (`IdentityFromContext`), since it's a plain `http.Handler` chain, not a long-lived session. The SDK ships **no** JWT verifier — both `/mcp` and `/api` are guarded by the *same* `internal/auth/local.go` `LocalVerifier` instance, which checks our own AS-issued tokens against a static in-process public key set (`oauthsrv.Service.PublicKeys()`) — no JWKS HTTP fetch, no dependency on our own public URL being reachable. It's constructed once in `cmd/server.Run` with `audiences = [{public_url}/mcp, {public_url}]`, so a token minted for either audience clears both routes. `internal/auth/oidc.go`'s `OIDCVerifier` is orphaned production code — upstream federation goes through `mcp-oauth`'s own `dex.NewProvider`, never this type — kept alive only by its own package tests; do not wire it into anything new. `LocalVerifier` is wrapped as the SDK's `TokenVerifier` in `internal/auth/resource.go`; `internal/auth/identity_pipeline.go`'s `VerifyRequestIdentity` is the shared verify→resolve call both `resource.go` and `internal/web/bearer.go` build on.
